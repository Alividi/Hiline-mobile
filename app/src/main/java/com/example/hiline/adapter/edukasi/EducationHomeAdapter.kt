...
class EducationHomeAdapter(
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): EducationHomeAdapter.MyViewHolder {
        val inflatedView: View = LayoutInflater.from(parent.context)
            .inflate(R.layout.recycleritem_main_user, parent, false)
        return MyViewHolder(inflatedView)
    }

    override fun onBindViewHolder(holder: EducationHomeAdapter.MyViewHolder, position: Int) {
        ...
        if (edukasiModels[position].result!! > 0){
            ...
            if (edukasiModels[position].color == "GREEN"){
                holder.clEdukasi.setBackgroundResource(R.drawable.mainuser_rv_bg_1)
                holder.tvKategori.setTextColor(context.resources.getColor(R.color.apricot))
            }
            if (edukasiModels[position].color == "Geraldine"){
                holder.clEdukasi.setBackgroundResource(R.drawable.mainuser_rv_bg_2)
            ...
            if (edukasiModels[position].color == "Periwinkle Blue"){
                holder.clEdukasi.setBackgroundResource(R.drawable.mainuser_rv_bg_3)
